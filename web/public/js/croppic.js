...
(function (window, document) {
	...
	Croppic.prototype = {
		...
		init: function () {
			...
			that.objW =  that.options.raiseObjSize * that.obj.width() ;
			that.objH = that.options.raiseObjSize * that.obj.height();
			...
			if( $.isEmptyObject(that.defaultImg)){
				...
				that.defaultImg = img.size() ? img : '';
				//maxim add code
				that.croppedImg = that.defaultImg;
			}
		...
		createImgUploadControls: function(){
			...
			var cropControlUpload = '';										// maxim change code
																			//<input id="upload-button" type="submit" class="submit cropControlUpload" value="Вибрати" title="Вибрати" />
			if(that.options.customUploadButtonId ===''){ cropControlUpload = '<i class="cropControlUpload" title="' + choosePhoto + '"></i>'; }
			var cropControlRemoveCroppedImage = '<i class="cropControlRemoveCroppedImage" title="' + removePhoto + '"></i>';


			if( $.isEmptyObject(that.defaultImg)){ cropControlRemoveCroppedImage=''; }
			if( !$.isEmptyObject(that.options.loadPicture)){ cropControlUpload='';}

			var html =    '<div class="custom-cropControls cropControlsUpload"> ' + cropControlUpload + cropControlRemoveCroppedImage + ' </div>';
		...
		afterUpload: function(data){
            ...
            if (response.status == 'success') {
                ...
                var img = $('<img src="'+response.url+'?' + Math.random()*1000000+1 +'" alt="' + photo + '">')
            ...
            if (response.status == 'ERROR' || response.status == 'REDIRECT' ) {
				that.hideLoader();
				if (that.options.onError) that.options.onError.call(handlers,response);
		...
		createModal: function(){
			...
			var marginTop = -that.objH/2;
			var modalHTML =  '<div id="croppicModal">'+'<div id="croppicModalObj" style="width:'+ that.objW  +'px; height:'+ that.objH +'px; top: 50%; margin:0 auto;  margin-top:'+ marginTop +'px; position: relative;"> </div>'+'</div>';
		...
		initDrag:function(){
			...
			var onOffCursor = function (action) {
				if (action) {
					that.modal.css({'cursor': navigator.userAgent.match(/Chrome|Opera|Safari/i) ? '-webkit-grabbing' : 'grabbing' });
					$('.cropImgWrapper').css({'cursor': navigator.userAgent.match(/Chrome|Opera|Safari/i) ? '-webkit-grabbing' : 'grabbing'});

				} else {
					that.modal.css({'cursor' : 'default'});
					$('.cropImgWrapper').css({'cursor': navigator.userAgent.match(/Chrome|Opera|Safari/i) ? '-webkit-grab': 'grab'});
				}
			}
			...
			that.img.on("mousedown touchstart", function(e) {
				...
				that.modal.on(e.type  == "mousedown" ? "mousemove" : "touchmove", function(e) {
					var imgTop;
					var imgLeft;
					...
					if (Math.abs(that.actualRotation)/90 % 2 != 0) {
						if (that.objH < that.imgH) {
							var start_pos = -( that.imgH - that.imgW ) / 2;
							if (parseInt(that.img.css('top')) > start_pos) {
								that.img.css('top', start_pos);
								if (that.options.imgEyecandy) {
									that.imgEyecandy.css('top', start_pos);
								}
							}
							var maxTop = -( (that.imgW - that.objH) + ( that.imgH - that.imgW ) / 2 );
						...
						if (that.objW < that.imgW) {
							var start_pos =  (that.imgH - that.imgW)/2 ;
							if (parseInt(that.img.css('left')) > start_pos) {
								that.img.css('left', start_pos);
								if (that.options.imgEyecandy) {
									that.imgEyecandy.css('left', start_pos);
								}
							}
							var maxLeft = -( (that.imgW-that.objW) + (that.imgH - that.imgW)/2);
			...
			that.modal.on("mouseup touchend", function(e) {
				that.modal.off("mousemove touchmove");
				onOffCursor(false);
			}).on("mouseout", function(e) {
				e = e || window.event;
				var relatedTarget = e.relatedTarget || e.fromElement;
				if ( !that.modal.is(relatedTarget) && that.modal.has( relatedTarget ).length === 0 ) {
					that.modal.off("mousemove touchmove");
					onOffCursor(false);
				}
			});
	    ...
	    rotate: function(x) {
			...
			that.img.css({
				...
				'-o-transform': 'rotate('+that.actualRotation + 'deg)',
				'-ms-transform': 'rotate('+that.actualRotation + 'deg)',
		...
		zoom :function(x){
			...
			if (Math.abs(that.actualRotation)/90 % 2 != 0) {
				...
				start_pos =  (that.imgH - that.imgW)/2 ;
		...
		crop:function(){
			...
			var cropData = {
					...
					imgW:that.imgW/that.options.raiseObjSize,
					imgH:that.imgH/that.options.raiseObjSize,
					cropH:that.objH/that.options.raiseObjSize,
					cropW:that.objW/that.options.raiseObjSize,
					rotation:that.actualRotation
				};
			cropData.imgX1 = (function() {
				if (Math.abs(that.actualRotation)/90 % 2 != 0) {
					return Math.abs( parseInt(that.img.css('left')) / that.options.raiseObjSize + (cropData.imgW-cropData.imgH)/that.options.raiseObjSize)
				} else {
					return Math.abs(parseInt(that.img.css('left'))) / that.options.raiseObjSize;
				}
			}());
			cropData.imgY1 = (function() {
				if (Math.abs(that.actualRotation)/90 % 2 != 0) {
					return Math.abs( parseInt(that.img.css('top')) / that.options.raiseObjSize + (cropData.imgH-cropData.imgW)/that.options.raiseObjSize)
				} else {
					return Math.abs( parseInt(that.img.css('top'))) / that.options.raiseObjSize;
				}
			}());
		...
		afterCrop: function (data) {
            ...
            if (response.status == 'success') {
                ...
                that.destroy();
				that.obj.removeClass('bg-photo');
                that.obj.append('<img class="croppedImg" src="' + response.url + '?' + Math.random()*1000000+1 + '">');
		...
		reset:function(){
			...
			if( !$.isEmptyObject(that.croppedImg)){ 
				that.obj.prepend(that.croppedImg);
