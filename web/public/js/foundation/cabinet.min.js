abinet.js:301: WARNING - constant location assigned a value more than once.
Original definition at externs.zip//browser/window.js:56
        location = cabinet;
        ^
...
$(this).addClass("expand"));$.ajax({type:"POST",url:"/settings/wndclose",data:"jsonData="+JSON.stringify({id_wnd:$(this).attr("id"),wnd_close:$(this).hasClass("minimize")?1:0}),dataType:"json",success:function(a){console.log(a)},error:function(a,b,c){alert("Ajax Error! Can't find path to file may be. "+a.status+" "+a.responseText+""+c.message)}});a.preventDefault();return!1});$(".description").tooltip();signup();recovery_password();remove_item();load_form();on_submit();auth_submit();handlers.timeouts.global_timeout=
setTimeout(function(){$("#results-bl .error, #results-bl .info").empty()},handlers.timeout);"/settings"==location.pathname&&(cropperRectangle=new Croppic("person-photo",{modal:!0,loaderHtml:'<div class="loader"><img width="47" height="47" src="/public/images/old/waiting.gif" alt="Waiting"></div>',rotateFactor:90,enableMousescroll:!0,imgEyecandyOpacity:.4,scaleToFill:!0,uploadUrl:"/user/uploadPhoto",cropUrl:"/user/cropPhoto",raiseObjSize:2,onError:function(a){handlers[a.status](a.result)},onBeforeRemoveCroppedImg:function(){$(".remove-dialog .first-paragraph").empty();
$(".remove-dialog .question").empty();$("#remove").attr("href","");$(".delete-popup-wrap .ui-dialog-titlebar .ui-dialog-title").text(titleRemove.photo);$(".remove-dialog .first-paragraph").text(photoTextRemove);$(".remove-dialog .question").text(photoQuestionRemove);$("#remove").attr("href","/user/removephoto");$(".remove-dialog").dialog("open")}}),$("#avatar").on({mouseenter:function(){$(".cropControlsUpload").slideDown("fast")},mouseleave:function(){$(".cropControlsUpload").slideUp("fast")}},"#person-photo"),
$("#phone").mask("+38 (999) 999-99-99",{placeholder:"_"}));$("#birthday").mask("99.99.9999",{placeholder:datePlaceholer});if("/person/search"==location.pathname){var a=function(a){var e=a?a:window.event;a=a.keyCode||a.which;if("keypress"!=e.type||c(e)){if("keyup"==e.type&&13!=a&&40!=a&&8!=a&&46!=a)return!1;d=0;$("#search input").val().replace(/\*/g,"").length<=b?(f&&clearTimeout(f),$("#wrap-result-search ").html("")):(f&&clearTimeout(f),f=setTimeout(function(){$("#search").submit()},500))}else if(13==
a)return!1},c=function(a){return null==a.which?32>a.keyCode?null:String.fromCharCode(a.keyCode):0!=a.which&&0!=a.charCode?32>a.which?null:String.fromCharCode(a.which):null};$("#search input.text-search").focus();var b=2,d=0,e=16,l=0,h=0,k=$(".content-t").height()+$(".content-c").height(),g=!1;$("#search").submit(function(a){$("#search input.loop-search").attr("src","/public/images/ui-anim_basic_16x16.gif");a=$(this);k<$(window).height()&&(e=4*Math.round(($(window).height()-k)/41+2),e=16<e?e:16);$.ajax({url:"/person/search",
type:"POST",data:a.serialize()+"&"+$.param({data:{start:d,count:e}}),cache:!1,success:function(a){if("USERS_FOUND"==a.status){0==d&&(h=a.result.totalCount,$("#wrap-result-search ").html(""));a=a.result.users;for(var b in a)l++,$('<div class="column media-object">').append('<a class="media-object-section" href="/person/card/'+a[b].value+'" ><img src="'+(a[b].mini_photo?"/user/getminiphoto/"+a[b].value:"/public/images/mini_user.png")+'" height="32" width="32"/></a><a class="media-object-section" href="/person/card/'+
a[b].value+'"><small>'+a[b].label+"</small></a>").appendTo($("#wrap-result-search ")).hide().slideToggle(500)}else"USERS_NOT_FOUND"==a.status&&0==d&&($("#wrap-result-search p").size()||$('<p class="primary text-center">').html("<b>"+a.result.label+"</b>").appendTo($("#wrap-result-search ").html("")).hide().slideToggle(500))},error:function(a,b,c){alert("Ajax Error! Can't find path to file may be. "+a.status+" "+c.message)},complete:function(){$("#search input.loop-search").attr("src","/public/images/search.png");
setTimeout(function(){g=!1},200)}});return!1});var f=null;$("#search input").bind({input:a,keypress:a,keyup:a});$(window).scroll(function(a){$(window).height()+$(window).scrollTop()>=$(document).height()-75&&!g&&(g=!0,0!=d&&h-d<=e||$("#search input").val().length<=b?clearTimeout(f):(d+=e,$("#search").submit()))})}});
